...
5. **整体 vs 拆分判断（必做）**：
   - **同一容器内的食物视为整体**：一碗面、一份盖饭、一碗汤等，不要拆分成食材逐一输出，直接以"牛肉面"、"番茄炒蛋盖饭"等整体名称输出一条，营养数值为该整体的合计值
   - **多个独立个体要分开**：同一盘里有 3 个馅饼，输出一条并标注 quantity 为 3；套餐里有汉堡、薯条、可乐，则各输出一条
   - 判断依据：看食物在物理上是否独立可分离。共处一个碗/盘且混合在一起的算整体；各自独立、可以单独拿起来的算多个主体
...
- **同一容器内的混合食物按整体输出**，不拆分食材（如一碗牛肉面输出一条"牛肉面"）
- **多个独立个体分开输出**（如套餐中的汉堡、薯条、可乐各一条；同类多个用 quantity 标注数量）
- 请严格按以下 JSON 格式返回，不要包含任何多余文本：
{
  ...
  /**
   * 快速分析提示词：直接分析食物营养，多食物主体时标记为复杂
   */
  private static readonly IMAGE_TRIAGE_PROMPT =
    VercelGatewayService.IMAGE_NUTRITION_PROMPT +
    `

## 多食物主体检测（额外规则）
在完成分析后，判断画面中**独立食物主体**的数量（每个碗、盘、杯、包装算一个主体）：
- 如果独立食物主体 ≤ 3 个，按上述格式正常输出分析结果
- 如果独立食物主体 ≥ 4 个，**不要输出完整分析**，只返回以下 JSON：
  { "complex": true, "reason": "画面中有N个独立食物主体，需要深度分析" }
`;
